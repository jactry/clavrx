...
!---------------------------------------------------------
  if (Level2_File_Flag == sym%YES) then
    ...
    call WRITE_CLAVRX_HDF_GLOBAL_ATTRIBUTES(Sd_Id_Level2,"PIXEL",File_Level2,File_1b_Root, &
     ...
     !---  surface height max
     if (Sds_Num_Level2_Zsfc_Max_3x3_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Zsfc_Max_3x3),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d,&
                              "surface_elevation_max_3x3", &
                              "surface_elevation_max_3x3", &
                              "maximum 3x3 surface elevation above mean sea level", &
                              DFNT_INT16, sym%LINEAR_SCALING, &
                              Min_Zsfc, Max_Zsfc, "meters", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     endif
     ...
     !---  surface height std
     if (Sds_Num_Level2_Zsfc_Std_3x3_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Zsfc_Std_3x3),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d,&
                              "surface_elevation_std_3x3", &
                              "surface_elevation_std_3x3", &
                              "standard deviation 3x3 pixels surface elevation above mean sea level", &
                              DFNT_INT16, sym%LINEAR_SCALING, &
                              Min_Zsfc_Std, Max_Zsfc_Std, "meters", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     endif
...
!------------------------------------------------------------------------------------------------------------------------
     ...
     if (Sds_Num_Level2_Ch1_Min_3x3_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(1) == sym%YES) then
       call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ch1_Min_3x3),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
     ...
     if (Sds_Num_Level2_Ch31_Max_3x3_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(31) == sym%YES) then
       call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ch31_Max_3x3),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                              "temp_11_0um_nom_max_3x3", &
                              "not specified", &
                              "maximum of the 11 micron brightness temperature "// &
                              "computed over a 3x3 pixel array", &
                              DFNT_INT16, sym%LINEAR_SCALING, &
                              Min_Bt31, Max_Bt31, "K", Missing_Value_Real4, Istatus)
       Istatus_Sum = Istatus_Sum + Istatus
     ...
     !--- Bt_Ch31_min_3x3
     if (Sds_Num_Level2_Ch31_Min_3x3_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(31) == sym%YES) then
       call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ch31_Min_3x3),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                              "temp_11_0um_nom_min_3x3", &
                              "not specified", &
                              "minimum of the 11 micron brightness temperature "// &
                              "computed over a 3x3 pixel array", &
                              DFNT_INT16, sym%LINEAR_SCALING, &
                              Min_Bt31, Max_Bt31, "K", Missing_Value_Real4, Istatus)
       Istatus_Sum = Istatus_Sum + Istatus
...
!---- Cloud Properties
     ...
     !--- binary cloud mask
     if (Sds_Num_Level2_Cld_Mask_Binary_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Cld_Mask_Binary),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d,&
                               "binary_cloud_mask", &
                               "not specified", &
                               "integer classification of the binary cloud mask including clear=0, cloudy=1", &
     ...
     if (Cld_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(1)==sym%YES .and. Sds_Num_Level2_Cod_Mask_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Cod_Mask),Sd_Id_Level2,Sds_Dims_2d, Sds_Chunk_Size_2d,&
                               ...
                               "and water phase with 10 micron particle size determined for cloud mask use", &
                                DFNT_INT16, sym%LINEAR_SCALING, Min_Tau_Mask, Max_Tau_Mask, &
     ...
     !--- cloud base altitude from acha
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Base_Alt_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Base_Alt),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                               "cld_base_altitude_acha", &
                               "altitude_at_cloud_base", &
                               "cloud base altitude computed from pressure values", &
                               DFNT_INT16, sym%LINEAR_SCALING, Min_Alt, Max_Alt, &
                               "feet", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     endif
     ...
     !--- cloud temperature from opaque
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Ctt_Opa_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ctt_Opa),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                              "cld_temp_opaque", &
     ...
     !--- cloud optical depth from DCOMP Mode 1
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Cod_Dcomp_1_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Cod_Dcomp_1),Sd_Id_Level2,Sds_Dims_2d, Sds_Chunk_Size_2d,&
                               "cld_opd_dcomp_1", &
                               "atmosphere_optical_thickness_due_to_cloud", &
                               "cloud optical depth at the nominal wavelength of 0.65 microns, "//&
                               "determined from DCOMP Mode 1 - NOAA CDR", &
                                DFNT_INT16, sym%LINEAR_SCALING, Min_tau, Max_tau, &
                               "none", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     endif

     !--- cloud optical depth from DCOMP Mode 2
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Cod_Dcomp_2_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Cod_Dcomp_2),Sd_Id_Level2,Sds_Dims_2d, Sds_Chunk_Size_2d,&
                               "cld_opd_dcomp_2", &
                               "atmosphere_optical_thickness_due_to_cloud", &
                               "cloud optical depth at the nominal wavelength of 0.65 microns, "//&
                               "determined from DCOMP Mode 2 - NOAA CDR", &
                                DFNT_INT16, sym%LINEAR_SCALING, Min_tau, Max_tau, &
                               "none", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     endif

     !--- cloud optical depth from DCOMP Mode 3
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Cod_Dcomp_3_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Cod_Dcomp_3),Sd_Id_Level2,Sds_Dims_2d, Sds_Chunk_Size_2d,&
                               "cld_opd_dcomp_3", &
                               "atmosphere_optical_thickness_due_to_cloud", &
                               "cloud optical depth at the nominal wavelength of 0.65 microns, "//&
                               "determined from DCOMP Mode 3 - NOAA CDR", &
                                DFNT_INT16, sym%LINEAR_SCALING, Min_tau, Max_tau, &
                               "none", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     endif
     ...
     !--- cloud effective particle radius from DCOMP Mode 1
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Ceps_Dcomp_1_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ceps_Dcomp_1),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                              "cld_reff_dcomp_1", &
                              "effective_radius_of_cloud_condensed_water_particles_at_cloud_top", &
                              "effective radius of cloud particles determined from DCOMP Mode 1; "//&
                              "see attributes for channels used - NOAA CDR", &
                              DFNT_INT16, sym%LINEAR_SCALING, Min_Reff, Max_Reff, &
                              "micron", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     ...
     !--- cloud effective particle radius from DCOMP Mode 2
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Ceps_Dcomp_2_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ceps_Dcomp_2),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                              "cld_reff_dcomp_2", &
                              "effective_radius_of_cloud_condensed_water_particles_at_cloud_top", &
                              "effective radius of cloud particles determined from DCOMP Mode 2; "//&
                              "see attributes for channels used - NOAA CDR", &
                              DFNT_INT16, sym%LINEAR_SCALING, Min_Reff, Max_Reff, &
                              "micron", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     ...
     !--- cloud effective particle radius from DCOMP Mode 3
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Ceps_Dcomp_3_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ceps_Dcomp_3),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                              "cld_reff_dcomp_3", &
                              "effective_radius_of_cloud_condensed_water_particles_at_cloud_top", &
                              "effective radius of cloud particles determined from DCOMP Mode 3; "//&
                              "see attributes for channels used - NOAA CDR", &
                              DFNT_INT16, sym%LINEAR_SCALING, Min_Reff, Max_Reff, &
                              "micron", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
...
!---- Non-cloud Properties
     ...
     !--- emiss 3.75
     if (Sds_Num_Level2_Ch20_Emiss_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ch20_Emiss),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                               "emiss_3_75um_nom", &
     ...
     !--- emiss 3.75 clear
     if (Sds_Num_Level2_Ch20_Emiss_Clear_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ch20_Emiss_Clear),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
     ...
     !--- emiss 3.75 median 3x3
     if (Sds_Num_Level2_Ch20_Emiss_Median_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ch20_Emiss_Median),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
     ...
     !--- emiss 3.75 median 3x3
     if (Sds_Num_Level2_Ch20_Bt_Median_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ch20_Bt_Median),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                              ...
                              "at the nominal wavelength of 3.75 microns", &
                              DFNT_INT16, sym%LINEAR_SCALING, &
                              Min_Bt20, Max_Bt20, "none", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     endif


     !--- bt 11 vs bt 6.7 covar 5x5
     if (Sds_Num_Level2_Bt_11_67_Covar_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(31) == sym%YES .and.  &
      ...
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Bt_11_67_Covar),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                              ...
                              DFNT_INT8, sym%LINEAR_SCALING, Min_Bt_Covar, Max_Bt_Covar, &
                              "none", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     endif
     ...
     if (Sds_Num_Level2_Btd_Ch31_Ch32_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(31) == sym%YES .and.  &
      ...
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Btd_Ch31_Ch32),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
     ...
     !--- Btd_Ch31_Ch27_Std_3x3
     if (Sds_Num_Level2_Btd_Ch31_Ch27_Std_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(31) == sym%YES .and.  &
         Sensor%Chan_On_Flag_Default(27) == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Btd_Ch31_Ch27_Std),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                              "diff_ch31_ch27_Std_3x3", &
                              "difference_11_minus_67_brightness_temperature_std_3x3", &
                              "3x3 pixel standard deviation of 11.0 micron minus 6.7 micron"// &
                              "brightness temperatures (nominal wavelengths)", &
                              DFNT_INT16, sym%LINEAR_SCALING, &
                              Min_Btd_Ch31_Ch27, Max_Btd_Ch31_Ch27, "K", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     endif
     ...
     !--- surface temperature uniformity background
     if (Sds_Num_Level2_Tsfc_Uni_Back_Flag == sym%YES) then
      call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Tsfc_Uni_Back),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                               "sst_background_uni_3x3", &
                               "sea_surface_skin_temperature_background_uni_3x3", &
                               "standard deviation of background sea surface skin temperature 3x3"// &
                               "surface temperature assumed using "// &
                               "ancillary data sources", &
                               DFNT_INT16, sym%LINEAR_SCALING, &
                               Min_Sst_std, Max_Sst_std, "none", Missing_Value_Real4, Istatus)
      Istatus_Sum = Istatus_Sum + Istatus
     endif
...
!---- quality flags
     ...
     !--- Ref_Ch1_Clear Min
     if (Sds_Num_Level2_Ch1_Clear_Min_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(1) == sym%YES) then
       call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ch1_Clear_Min),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                               "refl_0_65um_nom_clear_sky_min_3x3", &
                               "toa_bidirectional_reflectance_assuming_clear_sky_0_65_micron_nominal_min_3x3", &
                               "top of atmosphere bidirectional reflectance modeled assuming clear skies "// &
                               "minimum of 3x3 at the nominal wavelength of 0.65 microns", &
                                DFNT_INT16, sym%LINEAR_SCALING, &
                                Min_Ref_Ch1, Max_Ref_Ch1, "%", Missing_Value_Real4, Istatus)
       Istatus_Sum = Istatus_Sum + Istatus
     endif
     ...
     !--- Ref_Ch1_Clear Max
     if (Sds_Num_Level2_Ch1_Clear_Max_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(1) == sym%YES) then
       call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ch1_Clear_Max),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                               "refl_0_65um_nom_clear_sky_max_3x3", &
                               "toa_bidirectional_reflectance_assuming_clear_sky_0_65_micron_nominal_max_3x3", &
                               "top of atmosphere bidirectional reflectance modeled assuming clear skies "// &
                               "maximum of 3x3 at the nominal wavelength of 0.65 microns", &
                                DFNT_INT16, sym%LINEAR_SCALING, &
                                Min_Ref_Ch1, Max_Ref_Ch1, "%", Missing_Value_Real4, Istatus)
       Istatus_Sum = Istatus_Sum + Istatus
     endif
     ...
     !--- Ref_Ch1_Clear Std
     if (Sds_Num_Level2_Ch1_Clear_Std_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(1) == sym%YES) then
       call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ch1_Clear_Std),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                               "refl_0_65um_nom_clear_sky_std_3x3", &
                               "toa_bidirectional_reflectance_assuming_clear_sky_0_65_micron_nominal_std_3x3", &
                               "top of atmosphere bidirectional reflectance modeled assuming clear skies "// &
                               "standard deviation of 3x3 at the nominal wavelength of 0.65 microns", &
                                DFNT_INT16, sym%LINEAR_SCALING, &
                                Min_Ref_Ch1_Std, Max_Ref_Ch1_Std, "%", Missing_Value_Real4, Istatus)
       Istatus_Sum = Istatus_Sum + Istatus
     endif
     ...
     !--- Ch45 temperature Pseudo 13um Clear
     if (Sds_Num_Level2_Ch45_Clear_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(45) == sym%YES) then
       call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Ch45_Clear),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                              "temp_13_3um_nom_pseudo_clear_sky", &
                              "toa_brightness_temperature_assuming_clear_sky_13_3_micron_nominal_pseudo", &
                              "top of atmosphere brightness temperature modeled assuming clear skies "// &
                              "at the nominal wavelength of 13.3 microns pseudo", &
                              DFNT_INT16, sym%LINEAR_SCALING, &
                              Min_Bt45, Max_Bt45, "K", Missing_Value_Real4, Istatus)
       Istatus_Sum = Istatus_Sum + Istatus
     endif
     ...
     if (Sds_Num_Level2_BTD_11_12_NWC_Flag == sym%YES .and. &
       ...
       call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_BTD_11_12_NWC),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                               "btd_11um_12um_nom_nwc", &
                               "brightness_temperature_difference_11um_12um_nom_neighboring_warm_center", &
                               "11-12um brightness temperature difference at neighboring warm center", &
                                DFNT_FLOAT, sym%NO_SCALING, &
                                -100., 100., "1", Missing_Value_Real4, Istatus)
       Istatus_Sum = Istatus_Sum + Istatus
     endif

     !--- Sds_Num_Level2_EMISS_375_NWC  - WCS3
     if (Sds_Num_Level2_EMISS_375_NWC_Flag == sym%YES .and. &
          Sensor%Chan_On_Flag_Default(20) == sym%YES) then
       call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_EMISS_375_NWC),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                               "emiss_3_75um_nom_nwc", &
                               "emissivity_3_75um_nom_neighboring_warm_center", &
                               "3.75um emissivity at neighboring warm center", &
                                DFNT_FLOAT, sym%NO_SCALING, &
                                Min_Sfc_Ems, Max_Sfc_Ems, "1", Missing_Value_Real4, Istatus)
       Istatus_Sum = Istatus_Sum + Istatus
     endif

     !--- Sds_Num_Level2_Etrop_11um_LRC  - WCS3
     if (Sds_Num_Level2_Etrop_11um_LRC_Flag == sym%YES .and. &
          Sensor%Chan_On_Flag_Default(31) == sym%YES) then
       call DEFINE_PIXEL_2D_SDS(Sds_Id_Level2(Sds_Num_Level2_Etrop_11um_LRC),Sd_Id_Level2,Sds_Dims_2d,Sds_Chunk_Size_2d, &
                               "emissivity_11um_tropopause_lrc", &
                               "emissivity_11um_tropopause_local_radiative_center", &
                               "11um emissivity at the tropopause at local radiative center", &
                                DFNT_FLOAT, sym%NO_SCALING, &
                                Min_Etropo, Max_Etropo, "1", Missing_Value_Real4, Istatus)
       Istatus_Sum = Istatus_Sum + Istatus
     endif
...
!-------------------------------------------------------------------------
   if (Level2_File_Flag == sym%YES) then
      ...
      !--- surface elevation max
      if (Sds_Num_Level2_Zsfc_Max_3x3_Flag == sym%YES) then
          call SCALE_VECTOR_I2_RANK2(Sfc%Zsfc_Max,sym%LINEAR_SCALING,Min_Zsfc,Max_Zsfc,Missing_Value_Real4,Two_Byte_Temp)
          Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Zsfc_Max_3x3), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                    Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
      endif

      !--- surface elevation std
      if (Sds_Num_Level2_Zsfc_Std_3x3_Flag == sym%YES) then
          call SCALE_VECTOR_I2_RANK2(Sfc%Zsfc_Std,sym%LINEAR_SCALING,Min_Zsfc_Std,Max_Zsfc_Std,Missing_Value_Real4,Two_Byte_Temp)
          Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Zsfc_Std_3x3), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                    Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
      endif
...
!--------------------------------------------------------------------------------------------------
      ...
      !--- Bt_Ch31_Min_3x3
      if (Sds_Num_Level2_Ch31_Min_3x3_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(31) == sym%YES) then
       call SCALE_VECTOR_I2_RANK2(Bt_Ch31_Min_3x3,sym%LINEAR_SCALING,Min_Bt31,Max_Bt31, &
                                 Missing_Value_Real4,Two_Byte_Temp)
       Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Ch31_Min_3x3), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                      Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
      endif
...
!-------- cloud properties
     ...
     !--- cld mask binary
     if (Sds_Num_Level2_Cld_Mask_Binary_Flag == sym%YES) then
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Cld_Mask_Binary), Sds_Start_2d,Sds_Stride_2d,Sds_Edge_2d,     &
                        CLDMASK%Cld_Mask_Binary(:,Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
     endif
     ...
     if (Cld_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(1) == sym%YES .and. Sds_Num_Level2_Cod_Mask_Flag == sym%YES) then
      ...
      call SCALE_VECTOR_I2_RANK2(Temp_Pix_Array_1,sym%LINEAR_SCALING,Min_Tau_Mask,Max_Tau_Mask,Missing_Value_Real4,Two_Byte_Temp)
     ...
     !--- cld optical depth from dcomp mode 1
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Cod_Dcomp_1_Flag == sym%YES) then     
      call SCALE_VECTOR_I2_RANK2(Tau_DCOMP_1,sym%LINEAR_SCALING,Min_tau,Max_tau,Missing_Value_Real4,Two_Byte_Temp)
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Cod_Dcomp_1), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                        Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
     endif

     !--- cld optical depth from dcomp mode 2
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Cod_Dcomp_2_Flag == sym%YES) then     
      call SCALE_VECTOR_I2_RANK2(Tau_DCOMP_2,sym%LINEAR_SCALING,Min_tau,Max_tau,Missing_Value_Real4,Two_Byte_Temp)
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Cod_Dcomp_2), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                        Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
     endif

     !--- cld optical depth from dcomp mode 3
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Cod_Dcomp_3_Flag == sym%YES) then     
      call SCALE_VECTOR_I2_RANK2(Tau_DCOMP_3,sym%LINEAR_SCALING,Min_tau,Max_tau,Missing_Value_Real4,Two_Byte_Temp)
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Cod_Dcomp_3), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                        Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
     endif
     ...
     !--- cld particle size from dcomp mode 1
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Ceps_Dcomp_1_Flag == sym%YES) then     
      call SCALE_VECTOR_I2_RANK2(Reff_Dcomp_1,sym%LINEAR_SCALING,Min_Reff,Max_Reff,Missing_Value_Real4,Two_Byte_Temp)
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Ceps_Dcomp_1), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                        Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
     endif

     !--- cld particle size from dcomp mode 2
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Ceps_Dcomp_2_Flag == sym%YES) then     
      call SCALE_VECTOR_I2_RANK2(Reff_Dcomp_2,sym%LINEAR_SCALING,Min_Reff,Max_Reff,Missing_Value_Real4,Two_Byte_Temp)
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Ceps_Dcomp_2), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                        Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
     endif

     !--- cld particle size from dcomp mode 3
     if (Cld_Flag == sym%YES .and. Sds_Num_Level2_Ceps_Dcomp_3_Flag == sym%YES) then     
      call SCALE_VECTOR_I2_RANK2(Reff_Dcomp_3,sym%LINEAR_SCALING,Min_Reff,Max_Reff,Missing_Value_Real4,Two_Byte_Temp)
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Ceps_Dcomp_3), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                        Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
     endif
...
!--- non-cloud props     
    ...
    if (Sds_Num_Level2_Btd_Ch31_Ch27_Std_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(31) == sym%YES .and. &
        Sensor%Chan_On_Flag_Default(27) == sym%YES) then
      call SCALE_VECTOR_I2_RANK2(Btd_Ch31_Ch27_Std_3x3,sym%LINEAR_SCALING,Min_Btd_Ch31_Ch27, &
                       Max_Btd_Ch31_Ch27,Missing_Value_Real4,Two_Byte_Temp)
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Btd_Ch31_Ch27_Std), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                       Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
    endif
...
!--- quality flags
    ...
    !--- Ref_Ch1 clear min
    if (Sds_Num_Level2_Ch1_Clear_Min_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(1) == sym%YES) then
     call SCALE_VECTOR_I2_RANK2(Ref_Ch1_Clear_Min_3x3,sym%LINEAR_SCALING,Min_Ref_Ch1,Max_Ref_Ch1,Missing_Value_Real4,Two_Byte_Temp)
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Ch1_Clear_Min), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                       Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
    endif

    !--- Ref_Ch1 clear max
    if (Sds_Num_Level2_Ch1_Clear_Max_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(1) == sym%YES) then
     call SCALE_VECTOR_I2_RANK2(Ref_Ch1_Clear_Max_3x3,sym%LINEAR_SCALING,Min_Ref_Ch1,Max_Ref_Ch1,Missing_Value_Real4,Two_Byte_Temp)
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Ch1_Clear_Max), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                       Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
    endif

    !--- Ref_Ch1 clear std
    if (Sds_Num_Level2_Ch1_Clear_Std_Flag == sym%YES .and. Sensor%Chan_On_Flag_Default(1) == sym%YES) then
     call SCALE_VECTOR_I2_RANK2(Ref_Ch1_Clear_Std_3x3,sym%LINEAR_SCALING,Min_Ref_Ch1_Std,Max_Ref_Ch1_Std,Missing_Value_Real4,Two_Byte_Temp)
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Ch1_Clear_Std), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                       Two_Byte_Temp(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
    endif
    ...
    if (Sds_Num_Level2_BTD_11_12_NWC_Flag == sym%YES .and. &
      ...
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_BTD_11_12_NWC), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                      BTD_11_12um_NWC(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
    endif
    ...
    if (Sds_Num_Level2_EMISS_375_NWC_Flag == sym%YES .and. &
      ...
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_EMISS_375_NWC), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                      Emiss_39um_NWC(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
    endif

    !--- Sds_Num_Level2_Etrop_11um_LRC  - WCS3
    if (Sds_Num_Level2_Etrop_11um_LRC_Flag == sym%YES .and. &
         Sensor%Chan_On_Flag_Default(31) == sym%YES) then
      Istatus = sfwdata(Sds_Id_Level2(Sds_Num_Level2_Etrop_11um_LRC), Sds_Start_2d, Sds_Stride_2d, Sds_Edge_2d, &
                      Emiss_Tropo_11um_LRC(:, Line_Idx_Min_Segment:Sds_Edge_2d(2) + Line_Idx_Min_Segment - 1)) + Istatus
    endif
