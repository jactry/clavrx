!// $Id: coms_module.f90,v 1.4.2.2 2014/01/26 04:48:32 heidinger Exp $
!--------------------------------------------------------------------------------------
...
use VIEWING_GEOMETRY_MODULE
 ...
 implicit none
 public :: READ_COMS, READ_NAVIGATION_BLOCK_COMS, GET_COMS_IMAGE
...
!----------------------------------------------------------------------------------------------------
subroutine READ_COMS_DARK_COMPOSITE(Segment_Number,Dark_Composite_Filename, &
...
!           num_recs_to_read, Num_Scans_Read
   ...
   do Rec_Num = First_Rec_Num, Last_Rec_Num
     ...
     do Elem_Idx = 1, Num_Elements
        ii = min(Num_Elements_Dark,1 + COMS_Xstride*(Elem_Idx-1))
        if (Dark_Comp_Counts(ii) == Big_Count) then
           Ref_Ch1_Dark(Elem_Idx,Line_Idx) = Missing_Value_Real4
        else
           !Recall, COMS has 2 different flavors of reflectance calculation
           ! See COMS module for documentation
           index = int(Dark_Comp_Counts(ii),KIND=int2) + 1
           !---stw Ref_Ch1_Dark(Elem_Idx,Line_Idx) = (REAL(ref_table(2,1,index),KIND=real4) / 100.0)
           Ref_Ch1_Dark(Elem_Idx,Line_Idx) = ref_table(index)
...
end subroutine READ_COMS_DARK_COMPOSITE
