...
! input structure
 ...
 type, public :: acha_input_struct
   ...
   integer (kind=int4), dimension(:,:), allocatable:: Viewing_Zenith_Angle_Idx_Rtm
...
#if defined (ISFWAIT)
  type(AWG_CLX_CLOUD_HEIGHT_Ctxt), pointer :: Ctxt
  integer(LONG) :: Stat 
#endif
...
!----------------------------------------------------------------------
 subroutine  ACHA_Fetch_Pixel_NWP_RTM(Acha_Input, symbol, &
   ...
   if (Acha_Input%Chan_On_67um == sym%YES) then
     Acha_RTM_NWP%Atm_Rad_Prof_67um_RTM => Rtm(Inwp,Jnwp)%d(Ivza)%Rad_Atm_Clr_Ch27
     
     Acha_RTM_NWP%Atm_Trans_Prof_67um_RTM => Rtm(Inwp,Jnwp)%d(Ivza)%Trans_Atm_Clr_Ch27
     
   endif
   if (Acha_Input%Chan_On_85um == sym%YES) then
     Acha_RTM_NWP%Atm_Rad_Prof_85um_RTM => Rtm(Inwp,Jnwp)%d(Ivza)%Rad_Atm_Clr_Ch29
     
     Acha_RTM_NWP%Atm_Trans_Prof_85um_RTM => Rtm(Inwp,Jnwp)%d(Ivza)%Trans_Atm_Clr_Ch29
   endif
   if (Acha_Input%Chan_On_11um == sym%YES) then
      Acha_RTM_NWP%Atm_Rad_Prof_11um_RTM => Rtm(Inwp,Jnwp)%d(Ivza)%Rad_Atm_Clr_Ch31
      
      Acha_RTM_NWP%Atm_Trans_Prof_11um_RTM => Rtm(Inwp,Jnwp)%d(Ivza)%Trans_Atm_Clr_Ch31
      
      Acha_RTM_NWP%Black_Body_Rad_Prof_11um_RTM => Rtm(Inwp,Jnwp)%d(Ivza)%Cloud_Prof_Ch31
   endif
   
   if (Acha_Input%Chan_On_12um == sym%YES) then
      Acha_RTM_NWP%Atm_Rad_Prof_12um_RTM => Rtm(Inwp,Jnwp)%d(Ivza)%Rad_Atm_Clr_Ch32
      
      Acha_RTM_NWP%Atm_Trans_Prof_12um_RTM => Rtm(Inwp,Jnwp)%d(Ivza)%Trans_Atm_Clr_Ch32
   endif
   if (Acha_Input%Chan_On_133um == sym%YES) then
      Acha_RTM_NWP%Atm_Rad_Prof_133um_RTM => Rtm(Inwp,Jnwp)%d(Ivza)%Rad_Atm_Clr_Ch33
      
      Acha_RTM_NWP%Atm_Trans_Prof_133um_RTM => Rtm(Inwp,Jnwp)%d(Ivza)%Trans_Atm_Clr_Ch33
   endif
...
!AIT Framework
   ...
   !--- populate radiance and transmission profiles
   if (Acha_Input%Chan_On_67um == sym%YES) then
     CALL NFIA_RTM_Grid_RadAtmClr(Ctxt%RTM_Src1_T00, Elem_Idx, Line_Idx, CHN_ABI9, Acha_RTM_NWP%Atm_Rad_Prof_67um_RTM)
     CALL NFIA_RTM_Grid_TransAtmClr(Ctxt%RTM_Src1_T00, Elem_Idx, Line_Idx, CHN_ABI9, Acha_RTM_NWP%Atm_Trans_Prof_67um_RTM)     
   endif
   
   if (Acha_Input%Chan_On_85um == sym%YES) then
     CALL NFIA_RTM_Grid_RadAtmClr(Ctxt%RTM_Src1_T00, Elem_Idx, Line_Idx, CHN_ABI11, Acha_RTM_NWP%Atm_Rad_Prof_85um_RTM)
     CALL NFIA_RTM_Grid_TransAtmClr(Ctxt%RTM_Src1_T00, Elem_Idx, Line_Idx, CHN_ABI11, Acha_RTM_NWP%Atm_Trans_Prof_85um_RTM)
   endif
   
   if (Acha_Input%Chan_On_11um == sym%YES) then
      CALL NFIA_RTM_Grid_RadAtmClr(Ctxt%RTM_Src1_T00, Elem_Idx, Line_Idx, CHN_ABI14, Acha_RTM_NWP%Atm_Rad_Prof_11um_RTM)
      CALL NFIA_RTM_Grid_TransAtmClr(Ctxt%RTM_Src1_T00, Elem_Idx, Line_Idx, CHN_ABI14, Acha_RTM_NWP%Atm_Trans_Prof_11um_RTM)
      CALL NFIA_RTM_Grid_CloudProf(Ctxt%RTM_Src1_T00, Elem_Idx, Line_Idx, CHN_ABI14, Acha_RTM_NWP%Black_Body_Rad_Prof_11um_RTM)
   endif
   
   if (Acha_Input%Chan_On_12um == sym%YES) then
      CALL NFIA_RTM_Grid_RadAtmClr(Ctxt%RTM_Src1_T00, Elem_Idx, Line_Idx, CHN_ABI15, Acha_RTM_NWP%Atm_Rad_Prof_12um_RTM)
      CALL NFIA_RTM_Grid_TransAtmClr(Ctxt%RTM_Src1_T00, Elem_Idx, Line_Idx, CHN_ABI15, Acha_RTM_NWP%Atm_Trans_Prof_12um_RTM)
   endif
   
   
   if (Acha_Input%Chan_On_133um == sym%YES) then
      CALL NFIA_RTM_Grid_RadAtmClr(Ctxt%RTM_Src1_T00, Elem_Idx, Line_Idx, CHN_ABI16, Acha_RTM_NWP%Atm_Rad_Prof_133um_RTM)
      CALL NFIA_RTM_Grid_TransAtmClr(Ctxt%RTM_Src1_T00, Elem_Idx, Line_Idx, CHN_ABI16, Acha_RTM_NWP%Atm_Trans_Prof_133um_RTM)
   endif

   Ctxt => null()

!GEOCAT
#elif defined(ISGEOCAT)
   !--- do various 101 level NWP Profiles
      ...
      Line_Loop: DO Line=1, ACHA_Input%Number_of_Lines
         Elem_Loop: DO Elem=1, ACHA_Input%Number_of_Elements
            ...
            IF ((ACHA_Input%Latitude(Elem,Line) == Missing_Value_Real4) .OR. &
                (ACHA_Input%Longitude(Elem,Line) == Missing_Value_Real4)) THEN
               CYCLE
            ENDIF
